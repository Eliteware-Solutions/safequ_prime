    ...
    <div>
        ...
        <div style="padding: 1rem;padding-bottom: 0">
            <table>
                ...
                <tr>
                    <td class="strong">{{ $shipping_address ? $shipping_address->name : '' }}</td>
                </tr>
                @if (isset($order->user->user_community) && isset($order->user->user_community->name))
                    <tr>
                        <td class="strong">{{ $order->user->user_community->name }}</td>
                    </tr>
                @endif
                @if ($shipping_address)
                    @php
                        $city = trim($shipping_address->city) != '' ? ', ' . trim($shipping_address->city) : '';
                        $country = trim($shipping_address->country) != '' ? ', ' . trim($shipping_address->country) : '';
                        $postal_code = trim($shipping_address->postal_code) != '' ? ', ' . trim($shipping_address->postal_code) : '';
                    @endphp
                    <tr>
                        <td class="gry-color small">
                            {{ $shipping_address->address . $city . $country . $postal_code }}
                        </td>
                    </tr>
                    <tr>
                        ...
                        <td>
                            <table class="text-right sm-padding small strong">
                                <tbody>
                                    ...
                                    <tr>
                                        <th class="gry-color text-left">{{ translate('Service Charge') }}</th>
                                        <td class="currency">
                                            {{ single_price($order->service_charge) }}</td>
                                    </tr>
                                    <tr>
                                    ...
                                    <tr>
                                        ...
                                        <td class="currency">{{ single_price($order->grand_total + $order->service_charge) }}</td>
                                    </tr>
                                </tbody>
                            </table>
                        </td>
